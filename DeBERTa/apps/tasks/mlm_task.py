...
class NGramMaskGenerator:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            self.mask_prob + self.keep_prob <= 1
        ), f"The prob of using [MASK]({mask_prob}) and the prob of using original token({keep_prob}) should between [0,1]"
    ...
    def mask_tokens(self, tokens, rng, **kwargs):
        ...
        pvals = 1.0 / np.arange(1, self.max_gram + 1)
